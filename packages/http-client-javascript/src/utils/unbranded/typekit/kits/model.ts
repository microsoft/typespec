...
defineKit<TypeKit>({
  model: {
    ...
    getAccess(model) {
      // TODO: switch this over to typekits instead of calling out to tcgc. Figure out how to cache information
      const context = createTCGCContext($.program, "typescript");
      return getAccess(context, model);
    },
    getUsage(model) {
      const context = createTCGCContext($.program, "typescript");
      return getUsage(context, model);
    },
