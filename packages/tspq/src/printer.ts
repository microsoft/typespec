...
export function getTypeViewJson(program: Program, type: Type) {
  const location = getLocationInfo(program, type, process.cwd());
  ...
  return {
    ...
    kind: type.kind,
    namespace:
      "namespace" in type && type.namespace ? getNamespaceFullName(type.namespace) : undefined,
...
function getLocationInfo(program: Program, type: Type, cwd: string) {
