...
async def test_lro_begin_get_lro(client):
    result = await (
        await client.lro.begin_get_lro(
...
async def test_paging_post_action_paging(client):
    items = [
        item
        async for item in client.paging.post_action_paging(
            resource_group_name=RESOURCE_GROUP_NAME,
    ...
    assert (
        items[0].id
        == f"/subscriptions/{SUBSCRIPTION_ID}/resourceGroups/{RESOURCE_GROUP_NAME}/providers/Microsoft.Compute/virtualMachines/vm1"
    )
    assert items[0].sending_metrics is True
...
async def test_paging_mark_as_pageable(client):
    items = [
        item
        async for item in client.paging.mark_as_pageable(
            resource_group_name=RESOURCE_GROUP_NAME,
            monitor_name="monitor1",
        )
    ]
