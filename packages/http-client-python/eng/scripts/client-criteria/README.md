...
`@clientSurfaceDoc("…", "<category>")` — a language-agnostic description of what
...
**signatures.json** (mechanical routing table). A deterministic runner
(`verify.py`) checks everything with a signature; only fuzzy items go to the
`/check-client-surface` orchestrator prompt.

Whether a check is deterministic or AI is decided by whether the language
supplied a signature for its category — not by the scenario. Add a signature and
...
| Piece | Lives in | AI? |
| --- | --- | --- |
| `@clientSurfaceDoc` decorator | spector (with `@scenario`) | no |
| checks doc (precomputed manifest) | generated by spector, ships with specs | no |
| `context.md` + `signatures.json` (concept signatures) | each emitter repo | no (authored) |
| `verify.py` (deterministic runner) | shared | no |
| `/check-client-surface` orchestrator | monorepo `.github/prompts/` | AI only for the residual |

Work is pushed left so AI is the exception: (1) the `@clientSurfaceDoc` manifest
walk is deterministic; (2) concept signatures are authored once per language,
~a dozen entries reused across all scenarios; (3) `verify.py` decides every item
with a signature; (4) AI handles only the residual, cached on
...
and, per check, consults `signatures.json` — if the category is listed it runs
...
Then rerun with `signatures.promoted.json`, which adds the one `hierarchy`
...
generated-root, flavors, checks-doc path) plus a **concept signature** per
category, written concretely enough to execute as a file check. Optionally add
**signatures.json** (routing table mapping categories to `verify.py` routines)
