...
namespace Microsoft.Generator.CSharp
{
    ...
    public abstract class TypeFactory
    {
        ...
        {
            InputLiteralType literalType => CSharpType.FromLiteral(CreateCSharpType(literalType.ValueType),
                literalType.Value),
            InputUnionType unionType => CSharpType.FromUnion(
                unionType.UnionItemTypes.Select(CreateCSharpType).ToArray(), unionType.IsNullable),
            InputListType { IsEmbeddingsVector: true } listType => new CSharpType(typeof(ReadOnlyMemory<>),
                listType.IsNullable, CreateCSharpType(listType.ElementType)),
            InputListType listType => new CSharpType(typeof(IList<>), listType.IsNullable,
                CreateCSharpType(listType.ElementType)),
            InputDictionaryType dictionaryType => new CSharpType(typeof(IDictionary<,>), inputType.IsNullable,
                typeof(string), CreateCSharpType(dictionaryType.ValueType)),
            InputEnumType enumType => CodeModelPlugin.Instance.OutputLibrary.EnumMappings.TryGetValue(enumType,
                out var provider)
                ? provider.Type.WithNullable(inputType.IsNullable)
                : throw new InvalidOperationException(
                    $"No {nameof(EnumProvider)} has been created for `{enumType.Name}` {nameof(InputEnumType)}."),
            InputModelType model => CodeModelPlugin.Instance.OutputLibrary.ModelMappings.TryGetValue(model,
                out var provider)
                ? provider.Type.WithNullable(inputType.IsNullable)
            ...
            InputPrimitiveType primitiveType => primitiveType.Kind switch
            {
                ...
                InputPrimitiveTypeKind.Integer => new CSharpType(typeof(long),
                    inputType
                        .IsNullable), // in typespec, integer is the base type of int related types, see type relation: https://typespec.io/docs/language-basics/type-relations
                InputPrimitiveTypeKind.Float => new CSharpType(typeof(double),
                    inputType
                        .IsNullable), // in typespec, float is the base type of float32 and float64, see type relation: https://typespec.io/docs/language-basics/type-relations
                InputPrimitiveTypeKind.Numeric => new CSharpType(typeof(double),
                    inputType
                        .IsNullable), // in typespec, numeric is the base type of number types, see type relation: https://typespec.io/docs/language-basics/type-relations
        ...
        public virtual ModelProvider GetModelProvider(InputModelType inputModelType) => new ModelProvider(inputModelType);
        ...
        public virtual ParameterProvider GetParameterProvider(InputParameter parameter)
        {
            return new ParameterProvider(parameter);
        }

        /// <summary>
        /// Factory method for creating a <see cref="ParameterProvider"/> based on an input parameter <paramref name="property"/>.
        /// </summary>
        /// <param name="property">The <see cref="InputParameter"/> to convert.</param>
        /// <returns>An instance of <see cref="ParameterProvider"/>.</returns>
        public virtual ParameterProvider GetParameterProvider(InputModelProperty property)
        {
            return new ParameterProvider(property);
        }
