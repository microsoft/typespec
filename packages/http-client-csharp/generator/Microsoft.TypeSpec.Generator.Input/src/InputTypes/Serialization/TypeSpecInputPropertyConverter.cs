...
{
    internal sealed class TypeSpecInputPropertyConverter : JsonConverter<InputProperty>
    {
        private readonly TypeSpecReferenceHandler _referenceHandler;

        public TypeSpecInputPropertyConverter(TypeSpecReferenceHandler referenceHandler)
        {
        ...
        private static InputProperty CreatePropertyType(ref Utf8JsonReader reader, string? id, string? kind, JsonSerializerOptions options, ReferenceResolver resolver) => kind switch
        {
            ...
            ModelPropertyKind => TypeSpecInputModelPropertyConverter.ReadInputModelProperty(ref reader, id, kind, options, resolver),
            HeaderParameterKind => TypeSpecInputHeaderParameterConverter.ReadInputHeaderParameter(ref reader, id, kind, options, resolver),
            QueryParameterKind => TypeSpecInputQueryParameterConverter.ReadInputQueryParameter(ref reader, id, kind, options, resolver),
            PathParameterKind => TypeSpecInputPathParameterConverter.ReadInputPathParameter(ref reader, id, kind, options, resolver),
            BodyParameterKind => TypeSpecInputBodyParameterConverter.ReadInputBodyParameter(ref reader, id, kind, options, resolver),
