            ...
            else if (global::Sample.Optional.IsCollectionDefined(PropertyWithNestedArray))
            {
                ...
                for (int i = 0; (i < PropertyWithNestedArray.Count); i++)
                {
                    ...
                    for (int i0 = 0; (i0 < PropertyWithNestedArray[i].Count); i0++)
                    {
                        ...
                        {
                            if (((hasPatch1 && Patch.IsRemoved(global::System.Text.Encoding.UTF8.GetBytes($"$.propertyWithNestedArray[{i}][{i0}][{i1}]"))) || ((PropertyWithNestedArray[i][i0][i1] != null) && PropertyWithNestedArray[i][i0][i1].Patch.IsRemoved("$"u8))))
                            {
                                continue;
                            }
