...
namespace UnbrandedTypeSpec.Models
{
    ...
    public partial class ModelWithProjectedName : IJsonModel<ModelWithProjectedName>
    {
        ...
        ModelWithProjectedName IJsonModel<ModelWithProjectedName>.Create(ref Utf8JsonReader reader, ModelReaderWriterOptions options) => JsonModelCreateCore(ref reader, options);

        /// <param name="reader"> The JSON reader. </param>
        /// <param name="options"> The client options for reading and writing models. </param>
        protected virtual ModelWithProjectedName JsonModelCreateCore(ref Utf8JsonReader reader, ModelReaderWriterOptions options)
        {
            string format = options.Format == "W" ? ((IPersistableModel<ModelWithProjectedName>)this).GetFormatFromOptions(options) : options.Format;
            if (format != "J")
            {
                throw new FormatException($"The model {nameof(ModelWithProjectedName)} does not support reading '{format}' format.");
            }
            using JsonDocument document = JsonDocument.ParseValue(ref reader);
            return DeserializeModelWithProjectedName(document.RootElement, options);
        }

        internal static ModelWithProjectedName DeserializeModelWithProjectedName(JsonElement element, ModelReaderWriterOptions options)
        {
            ...
            return new ModelWithProjectedName(name, serializedAdditionalRawData);
        }
        ...
        ModelWithProjectedName IPersistableModel<ModelWithProjectedName>.Create(BinaryData data, ModelReaderWriterOptions options) => PersistableModelCreateCore(data, options);

        /// <param name="data"> The data to parse. </param>
        /// <param name="options"> The client options for reading and writing models. </param>
        protected virtual ModelWithProjectedName PersistableModelCreateCore(BinaryData data, ModelReaderWriterOptions options)
        {
            string format = options.Format == "W" ? ((IPersistableModel<ModelWithProjectedName>)this).GetFormatFromOptions(options) : options.Format;
            switch (format)
            {
                ...
                default:
                    throw new FormatException($"The model {nameof(ModelWithProjectedName)} does not support reading '{options.Format}' format.");
            }
        }
