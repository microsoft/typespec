// <auto-generated/>

#nullable disable

using System.Collections.Generic;

namespace _Type.Model.Visibility.Models
{
    public static partial class TypeModelVisibilityModelFactory
    {
        public static VisibilityModel VisibilityModel(string readProp = default, int queryProp = default, IEnumerable<string> createProp = default, IEnumerable<int> updateProp = default, bool deleteProp = default, VisibilityModelNoneProp noneProp = default) => throw null;

        public static ReadOnlyModel ReadOnlyModel(IEnumerable<int> optionalNullableIntList = default, IReadOnlyDictionary<string, string> optionalStringRecord = default) => throw null;
    }
}
