...
{
    internal class TestPluginMethodProviderCollection : MethodProviderCollection
    {
        public TestPluginMethodProviderCollection(InputOperation operation, TypeProvider enclosingType)
        ...
        private static MethodProvider BuildProtocolMethod(InputOperation operation, TypeProvider enclosingType, bool isAsync)
        {
            ...
            foreach (InputParameter inputParam in operation.Parameters)
            {
                if (inputParam.Kind != InputOperationParameterKind.Method)
                    continue;
                methodParameters.Add(TestCodeModelPlugin.Instance.TypeFactory.CreateCSharpParam(inputParam));
            }
        ...
        private static CSharpType? GetResponseType(IReadOnlyList<OperationResponse> responses, bool isAsync)
        {
            ...
            if (response is null || response.BodyType is null)
                return null;
            var returnType = TestCodeModelPlugin.Instance.TypeFactory.CreateCSharpType(response.BodyType);
            if (isAsync)
            {
