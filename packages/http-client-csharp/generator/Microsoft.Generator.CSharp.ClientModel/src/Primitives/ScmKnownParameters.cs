...
namespace Microsoft.Generator.CSharp.ClientModel.Primitives
{
    internal static class ScmKnownParameters
    {
        ...
        public static readonly ParameterProvider RequestOptions = new("options", $"The request options, which can override default behaviors of the client pipeline on a per-call basis.", ClientModelPlugin.Instance.TypeFactory.HttpRequestOptionsType);
        public static readonly ParameterProvider RequestContent = new("content", $"The content to send as the body of the request.", ClientModelPlugin.Instance.TypeFactory.RequestContentType, location: ParameterLocation.Body) { Validation = ParameterValidationType.AssertNotNull };
