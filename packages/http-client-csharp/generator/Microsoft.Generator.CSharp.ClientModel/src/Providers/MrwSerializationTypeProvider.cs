    ...
    internal sealed class MrwSerializationTypeProvider : TypeProvider
    {
        ...
        internal MethodProvider BuildJsonModelCreateCoreMethod()
        {
            ...
            var methodBody = new MethodBodyStatement[]
            {
                ...
                UsingDeclare("document", typeof(JsonDocument), JsonDocumentSnippet.ParseValue(_utf8JsonReaderParameter), out var docVariable),
                // return DeserializeT(doc.RootElement, options);
                Return(TypeProviderSnippet.Deserialize(_model, new JsonDocumentSnippet(docVariable).RootElement, _mrwOptionsParameterSnippet))
            };
        ...
        private ValueExpression CreateDeserializeValueExpression(CSharpType valueType, SerializationFormat serializationFormat, JsonElementSnippet jsonElement) =>
            valueType switch
            {
                ...
                { Implementation: ModelProvider modelProvider } =>
                    TypeProviderSnippet.Deserialize(modelProvider, jsonElement, _mrwOptionsParameterSnippet),
