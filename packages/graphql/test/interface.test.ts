...
describe("@compose", () => {
  ...
  it("Can spread properties from the interface", async () => {
    const diagnostics = await Tester.diagnose(`
      @graphqlInterface model AnInterface {
        prop: string;
      }
  ...
  it("Requires that an implemented model is an Interface", async () => {
    ...
    expectDiagnostics(diagnostics, {
      ...
      message:
        "All models used with `@compose` must be marked with `@graphqlInterface`, but NotAnInterface is not.",
    });
  });
