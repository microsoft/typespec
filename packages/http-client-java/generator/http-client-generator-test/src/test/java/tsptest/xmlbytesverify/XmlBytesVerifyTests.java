 ...
 * structured model) must NOT generate the static XML {@code ObjectSerializer XML_SERIALIZER} field, because the
...
public class XmlBytesVerifyTests {
    ...
    private static boolean hasSerializerField(Class<?> clazz) {
        return Arrays.stream(clazz.getDeclaredFields()).anyMatch(f -> "XML_SERIALIZER".equals(f.getName()));
    }

    @Test
    public void noXmlSerializerFieldForBytesPayload() {
        Assertions.assertFalse(hasSerializerField(XmlBytesVerifyClient.class),
            "XmlBytesVerifyClient should not declare a static XML_SERIALIZER field for a raw bytes XML payload.");
        Assertions.assertFalse(hasSerializerField(XmlBytesVerifyAsyncClient.class),
            "XmlBytesVerifyAsyncClient should not declare a static XML_SERIALIZER field for a raw bytes XML payload.");
    }
    ...
    public void xmlSerializerFieldPresentForModelPayload() throws ClassNotFoundException {
        ...
        // check above is meaningful and would catch a regression that emits XML_SERIALIZER unconditionally.
        ...
        Field serializer = Arrays.stream(modelClient.getDeclaredFields())
            .filter(f -> "XML_SERIALIZER".equals(f.getName()))
            .findFirst()
            .orElse(null);
        Assertions.assertNotNull(serializer,
            "payload.xml.SimpleModelValueClient should declare a XML_SERIALIZER field.");
    }
}
