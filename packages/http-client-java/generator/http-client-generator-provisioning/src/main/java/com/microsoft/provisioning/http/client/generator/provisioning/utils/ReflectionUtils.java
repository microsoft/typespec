...
public class ReflectionUtils {
    ...
    public static List<String> getEnumValues(Class<?> type) {
        ...
        return Arrays.stream(type.getDeclaredFields()).map(field -> getMemberValue(type, field)).toList();
    }
        ...
        return properties.stream()
            ...
            .flatMap(property -> {
                ...
                if (property.getPropertyType() instanceof ListModel) {
                    ...
                    imports.add(((ListModel) property.getPropertyType()).getElementType().getProvisioningPackage() + "."
                        + ((ListModel) property.getPropertyType()).getElementType().getName());
                ...
                if (property.getPropertyType() instanceof DictionaryModel) {
                    ...
                    imports.add(((DictionaryModel) property.getPropertyType()).getElementType().getProvisioningPackage()
                        + "." + ((DictionaryModel) property.getPropertyType()).getElementType().getName());
                    imports.add("com.azure.provisioning.BicepDictionary");
                    return imports.stream();
                }
                imports.add(
                    property.getPropertyType().getProvisioningPackage() + "." + property.getPropertyType().getName());
                return imports.stream();
            })
