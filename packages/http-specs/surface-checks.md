...
| Serialization_EncodedName_Json_Property_send_naming           | Serialization_EncodedName_Json_Property_send        | naming    | JsonEncodedNameModel.defaultName |       | expected=defaultName; kind=property | The client property is named `defaultName` even though the wire name is `wireName`. |
