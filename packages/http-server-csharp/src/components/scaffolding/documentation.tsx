...
export interface DocumentationProps {
  /** Names of interfaces with mock implementations. */
  interfaceNames: string[];
  /** Whether SwaggerUI is enabled. */
  useSwaggerUI?: boolean;
}

/**
 * Renders documentation files: README.md, docs/usage.md, docs/emitter.md.
 */
export function Documentation(props: DocumentationProps): Children {
  const hasMocks = props.interfaceNames.length > 0;
  const readme = hasMocks ? getMockReadme(props.interfaceNames) : getNoMockReadme();

  return (
    <>
      <SourceFile filetype="md" path="README.md">
        {readme}
      </SourceFile>
      <SourceDirectory path="docs">
        <SourceFile filetype="md" path="usage.md">
          {getUsageDoc()}
        </SourceFile>
        <SourceFile filetype="md" path="emitter.md">
          {getEmitterDoc()}
        </SourceFile>
      </SourceDirectory>
    </>
...
function getMockReadme(interfaceNames: string[]): string {
  const mockLines = interfaceNames.map((name) => `  - \`mocks/${name}.cs\``).join("\n");
  return `---
title: Next Steps
---
