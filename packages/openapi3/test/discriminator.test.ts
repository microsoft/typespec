...
worksFor(["3.0.0", "3.1.0"], ({ checkFor, openApiFor }) => {
  ...
  it("discriminator can be an inline union", async () => {
    const openApi = await openApiFor(`
      @discriminator("type")
      ...
      `);
    deepStrictEqual(openApi.components.schemas.ResponseFormat, {
      type: "object",
      properties: {
        type: {
          type: "string",
      ...
      discriminator: {
        ...
        mapping: {
          ...
          json_object: "#/components/schemas/ResponseFormatJson",
          json_schema: "#/components/schemas/ResponseFormatJsonSchema",
        },
      },
    });
    deepStrictEqual(openApi.components.schemas.ResponseFormatText.allOf, [
      { $ref: "#/components/schemas/ResponseFormat" },
    ]);
    deepStrictEqual(openApi.components.schemas.ResponseFormatJson.allOf, [
      { $ref: "#/components/schemas/ResponseFormat" },
    ]);
    deepStrictEqual(openApi.components.schemas.ResponseFormatJsonSchema.allOf, [
      { $ref: "#/components/schemas/ResponseFormat" },
    ]);
  });

  it("discriminator with inline union in operation", async () => {
    const openApi = await openApiFor(`
      @discriminator("type")
      model ResponseFormat {
