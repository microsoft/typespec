...
export async function $onEmit(context: EmitContext<CSharpServiceEmitterOptions>) {
  ...
  class CSharpCodeEmitter extends CodeTypeEmitter {
    ...
    #emitUsings(): EmitterOutput<string> {
      const builder: StringBuilder = new StringBuilder();
      for (const ns of [...this.emitter.getContext().file.imports.keys()])
    ...
    modelDeclarationContext(model: Model, name: string): Context {
      ...
      return this.#createModelContext(modelNamespace, modelFile, model);
    }
    ...
    #emitResponseType(type: Type) {
      switch (type.kind) {
        ...
        case "Intrinsic":
          if (isVoidType(type) || isNullType(type)) return "void";
          return "object";
        default:
          return "object";
      }
    ...
    #emitKnownTypeReference(model: Model): string | StringBuilder {
      ...
      return code`${this.emitter.emitTypeReference(model)}`;
    }
    ...
    #createModelContext(
      ...
      const context = {
       ...
       context.file.imports.set("System.Collections.Threading", ["System.Collections.Threading"]);
       context.file.imports.set("System.Text.Json", ["System.Text.Json"]);
       context.file.imports.set("System.Text.Json.Serialization", ["System.Text.Json.Serialization"]);
      return context;
    }
