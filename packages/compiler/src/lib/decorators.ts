...
export const $strictExtends: StrictExtendsDecorator = (
  ...
  // Validate once the whole type graph is checked. Validating earlier would look at a union that is
  // still being built when the type graph is cyclic, and would let a decorator applied after this
  // one change the variants and silently break the guarantee.
  // For a union created after the type graph was checked, a clone a mutator produced during
  // `$onValidate` for example, the checker runs this as soon as the union is finished instead.
  return { onGraphFinish: () => validateStrictExtends(context, entity) };
};
