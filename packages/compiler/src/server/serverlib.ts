...
export function createServer(host: ServerHost): Server {
  ...
  async function getFilePathChangedMap(changes: FileEvent[]): Promise<Map<string, string>> {
    const tspChanges = changes.filter((change) => change.uri.endsWith(".tsp"));
    const filePathChangedMap = new Map<string, string>();

    /**
     * The length of changes is even in order to exclude the following situations:
     * 1. Save only after modifying the content of a file;
     * 2. Delete only a file;
     * 3. Create only a file;
     */
    if (tspChanges.length === 0 || tspChanges.length % 2 !== 0) {
      return filePathChangedMap;
    }
    ...
    if (tspChanges.length === 2) {
      ...
      const oldFilePath = tspChanges.find((c) => c.type === FileChangeType.Deleted);
      const newFilePath = tspChanges.find((c) => c.type === FileChangeType.Created);
      ...
      // Move the file(s) to a new location,
      for (const change of tspChanges) {
        if (change.type === FileChangeType.Created) {
          ...
          const oldChange = tspChanges.find(
            (c) => c.type === FileChangeType.Deleted && c.uri.endsWith(fileName),
          );
          if (oldChange) {
    ...
    // Verify that the 'filePathChangedMap' generated by the parameter is ultimately in line with expectations,
    // otherwise the addition of logs will be convenient for debugging
  ...
  async function updateImportsOnFileChange(changes: FileEvent[]): Promise<void> {
    const filePathChangedMap = await getFilePathChangedMap(changes);
