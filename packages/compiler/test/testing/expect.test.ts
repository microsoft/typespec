...
import { expectDiagnostics, type ExpectDiagnosticsOptions } from "../../src/testing/index.js";
...
it("can match diagnostics regardless of order", () => {
  ...
  const options: ExpectDiagnosticsOptions = { fixedOrder: false };

  expectDiagnostics(diagnostics, [{ code: "second" }, { code: "first" }], options);
});
