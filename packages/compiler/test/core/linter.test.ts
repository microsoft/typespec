...
describe("compiler: linter", () => {
  async function createTestLinter(code: string, linterDef: LinterDefinition): Promise<Linter> {
  ...
  it("enabling a rule from a library that is not found emit a diagnostic", async () => {
    ...
    expectDiagnostics(
      ...
      }),
      {
        ...
        message: `Rule set "no-model-foo" is not found in library "@typespec/not-a-linter"`,
      }
    );
  });
