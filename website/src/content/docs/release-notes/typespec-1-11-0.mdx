...
Spector now includes semantic matchers for values like datetimes, and query/header matching now preserves matcher objects instead of stringifying them before comparison. That makes cross-language scenario tests less sensitive to formatting differences in timestamps and similar values.
...
When one config option references another, the compiler now leaves non-primitive values untouched instead of stringifying arrays or objects into unexpected values. This makes advanced emitter configuration more predictable when emitters need to resolve structured options themselves.
