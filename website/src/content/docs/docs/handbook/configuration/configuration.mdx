...
A `tspconfig.yaml` file can be marked as a **project configuration** by setting `kind: project` at the top level:
...
A project configuration marks a **project boundary** — it serves the same role as `package.json` does today for boundary detection, but is specific to TypeSpec. When the compiler or IDE encounters a `tspconfig.yaml` with `kind: project`, it treats that directory as the root of a TypeSpec project.

#### `kind`

Set to `"project"` to mark the directory as a TypeSpec project root.

**Constraints:**

- `kind: project` can only be used in files named `tspconfig.yaml`. Custom-named config files (e.g. used with `--config`) cannot be project configs.

#### `entrypoint`

Declares the main TypeSpec file for the project. This is used by both the CLI (`tsp compile <dir>`) and the IDE (language server) to determine which file to compile.

- **Default:** `main.tsp` (when `kind: project` is set but no `entrypoint` is specified)
- Can only be used when `kind: project` is set.
...
When compiling a directory, the compiler resolves the entrypoint in this order:

1. **Project tspconfig** — If the directory has a `tspconfig.yaml` with `kind: project`, use its `entrypoint` (defaults to `main.tsp`)
2. **package.json `tspMain`** — If a `package.json` exists with a `tspMain` field, use that
3. **`main.tsp`** — Fall back to `main.tsp` in the directory

The IDE language server uses the same priority when walking up directories to find the project root for a given TypeSpec file. When a project boundary (`kind: project`) is found, the walk stops — files outside the project boundary are not considered.
...
model LinterConfig {
  extends?: RuleSetRef[];
  enable?: Record<RuleRef, boolean | object>;
...
- Different emitters from both files are combined
- If the same emitter is configured in both files, the extending file's emitter configuration completely overrides the extended file's configuration for that emitter
- Individual emitter options are not merged - it's all or nothing per emitter

**Example:**

```yaml
# Base config (parent/tspconfig.yaml)
emit:
  - emitter1
options:
  emitter1:
...
# Extending config (child/tspconfig.yaml)
extends: ../tspconfig.yaml
emit:
  - emitter3 # Completely replaces the emit array
options:
  emitter1: # Completely replaces emitter1 options
    option-c: value-c
  emitter3: # Added alongside emitter2 from parent
    option-y: value-y

# Resulting merged config:
# emit: [emitter3]  # Only emitter3, emitter1 is gone
# options:
#   emitter1: { option-c: value-c }  # Only option-c, option-a and option-b are gone
#   emitter2: { option-x: value-x }  # Inherited from parent
#   emitter3: { option-y: value-y }  # From extending config
```
...
| --------------- | ------------------------- | -------------------------------------------------------- |
| `kind`          |                           | Set to `"project"` to mark as a project boundary         |
| `entrypoint`    |                           | Main TypeSpec file (requires `kind: project`)            |
...
You can also reference rules and rulesets using their **short name**, where the package scope is stripped (`@typespec/<name>` &rarr; `<name>`, `@<scope>/typespec-<name>` &rarr; `<name>`) or replaced by a library-declared `alias`. The library must be loaded (imported) for its short name to be resolved. The full name is always accepted.
...
linter:
  ...
  enable:
    "best-practices/no-x": true
```

If two loaded libraries would resolve to the same short name, that short name is ambiguous: using it reports a warning and the entry is skipped, so the full name must be used for those libraries.

#### Configuring rule options

Some rules accept options to customize their behavior. To pass options, use an object instead of `true` when enabling the rule:

```yaml
linter:
  enable:
    # Enable with default options
    "@typespec/my-linter/required-doc": true

    # Enable with custom options
    "@typespec/my-linter/naming-convention":
      style: "PascalCase"
      allowUnderscores: false
```

When an object is provided, the values are merged with the rule's default options. See each rule's documentation for available options.

#### Extending a ruleset defined in a file

A ruleset can also be defined in a standalone YAML file and referenced with the `file:` prefix. This is useful to share a common set of rules across multiple projects, or to version a ruleset independently of the libraries providing the rules.
...
The referenced file has the same shape as the `linter` config:

```yaml title=common-rules.yaml
extends:
  - "@typespec/best-practices/recommended"
...
`file:` references can only be used in `tspconfig.yaml` or in another ruleset file. A ruleset defined in a library cannot use them, as it is not anchored to any directory the path could be resolved against.
...
Specify a different config file for build overrides.
...
:::note
`--config` cannot point to a project configuration (one with `kind: project`). Project configs must be named `tspconfig.yaml` and are auto-discovered. Use `--config` with a non-project build config that can optionally use `extends` to inherit from the project config.

When using `--config`, project-specific settings (like `entrypoint`) are automatically inherited from the project `tspconfig.yaml` that would normally apply to the entrypoint — the same project config that would be found if `--config` was not specified. Build settings (like `emit`, `options`, `linter`) are **not** inherited — use `extends` for that.
:::
