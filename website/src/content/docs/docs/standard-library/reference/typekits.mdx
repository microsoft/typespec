 ...
 */
$(program).value
  .isOfType(source: Value, target: Type, diagnosticTarget?: Entity | Node): boolean;
$(program).value.isOfType
  .withDiagnostics(source: Value, target: Type, diagnosticTarget?: Entity | Node): [boolean, readonly Diagnostic[]];
```
