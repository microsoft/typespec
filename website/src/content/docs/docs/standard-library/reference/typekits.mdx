 ...
 */
$(program).model.getEffectiveModel(model: Model,
filter: (property: ModelProperty) => boolean): Model;
```
 ...
 */
$(program).model.getProperties(model: Model,
options: {
 ...
 */
$(program).modelProperty.getVisibilityForClass(property: ModelProperty,
visibilityClass: Enum): Set<EnumMember>;
```
 ...
 */
$(program).union.filter(union: Union,
filterFn: (variant: UnionVariant) => boolean): Union;
```
