...
function tokens(text: string) {
  ...
  do {
    ...
    result.push({
      ...
      v: scanner.value,
      s: scanner.state,
      x: scanner.positionFromOffset(scanner.offset)
    });
  } while (scanner.notEof);

  // verify that the input matches the output 
  const out = result.map(each => each.v).join('');
...
@suite class TestScanner {
  ...
  @test 'smoketest'() {
    ...
    verify(all, [
      { t: Kind.Whitespace },
      { t: Kind.Identifier, v: 'this' },
      { t: Kind.Whitespace },
      { t: Kind.Identifier, v: 'is' },
      { t: Kind.Whitespace },
      { t: Kind.Identifier, v: 'a' },
      { t: Kind.Whitespace },
      { t: Kind.Identifier, v: 'test' },
    ])
  }
