...
When metadata is deemed "inapplicable", for example, if a `@path` property is seen in a response, it becomes part of the payload instead unless the [@includeInapplicableMetadataInPayload][metadata](./decorators.md#includeinapplicablemetadatainpayload) decorator is used and given a value of `false`.
