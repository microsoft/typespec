...
In certain situations where the body type is not (necessarily) a model or the model has decorators like `@discriminator`, you will need to use the new `Body<T>` type. For example.
