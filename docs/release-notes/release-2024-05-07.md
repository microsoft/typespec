...
In TypeSpec decorators often expect some sort of configuration to be passed. In version `0.51.0` we introduce the `valueof` keyword to constraint a decorator or template parameter to be a value, however this was limted to string, numeric and boolean values.
